...
module.exports = class Callout extends Base
  STATIC_ASSETS: ['style.css']

  constructor: (args...) ->
    super(args...)

    @sourceAssets = path.join __dirname, 'callout'
  ...
  renderCompleted: (callback) ->
    @log.trace 'styles.Callout#renderCompleted(...)'
  ...
  copyAssets: (callback) ->
    @log.trace 'styles.Callout#copyAssets(...)'
  ...
  compileScript: (callback) ->
    @log.trace 'styles.Callout#compileScript(...)'
  ...
  compressScript: (scriptSource, callback) ->
    @log.trace 'styles.Callout#compressScript(..., ...)'
  ...
  concatenateScripts: (scriptSource, callback) ->
    @log.trace 'styles.Callout#concatenateScripts(..., ...)'
